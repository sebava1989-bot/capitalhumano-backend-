...
router.post('/generate', verifyAdmin, async (req, res) => {
  ...
  try {
    ...
    if (type === 'antiguedad') {
      ...
      specificText = `El trabajador tiene ${diasVacaciones} dia${diasVacaciones !== 1 ? 's' : ''} habiles de vacaciones disponibles, correspondientes a ${antiguedadTexto} de servicio en la empresa.`;
    } else if (type === 'renta') {
...
// POST /api/certificates/request — el trabajador solicita su propio certificado
router.post('/request', verifyWorker, async (req, res) => {
  ...
  // Reutilizar lógica de generación pasando workerId y companyId del token
  ...
  try {
    ...
    page.drawRectangle({ x: 50, y: startY - lineH * 6 - 20, width: width - 100, height: lineH * 4 + 20, color: rgb(0.97, 0.97, 0.97), borderColor: rgb(0.85, 0.85, 0.85), borderWidth: 1 });
    const fields = [
      ['Nombre completo', worker.full_name],
      ['RUT', worker.rut],
      ['Cargo', worker.position || 'No especificado'],
    ...
    page.drawText(specificText, { x: 50, y: startY - lineH * 7 - 10, size: 11, font: fontRegular, color: darkGray });
    ...
    const pdfBytes = await pdfDoc.save();
    const cloudResult = await uploadBuffer(Buffer.from(pdfBytes), {
      resource_type: 'raw',
      folder: `capitalhumano/${req.companyId}/certificates`,
