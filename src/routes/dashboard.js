...
// GET /api/dashboard/max-advance — leer monto máximo anticipo
router.get('/max-advance', verifyAdmin, async (req, res) => {
  try {
...
// PUT /api/dashboard/max-advance — actualizar monto máximo anticipo
router.put('/max-advance', verifyAdmin, async (req, res) => {
  const { maxAmount } = req.body;
  const amt = parseFloat(maxAmount);
  if (isNaN(amt) || amt < 0) return res.status(400).json({ error: 'Monto inválido' });
  try {
    await pool.query(
      'UPDATE companies SET max_advance_amount=$1 WHERE id=$2',
      [amt, req.companyId]
    );
    res.json({ ok: true, maxAmount: amt });
  } catch (err) {
    res.status(500).json({ error: 'Error interno' });
  }
});
